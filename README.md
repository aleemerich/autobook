...
> [!NOTE]
> **Sobre o Projeto**: Este projeto é baseado e inspirado no projeto original [autonovel](https://github.com/NousResearch/autonovel) da equipe NousResearch. Ele foi estendido e aprimorado para suportar uma arquitetura multi-LLM (OpenRouter, Gemini, OpenAI e Anthropic) flexível, robusta e interoperável para geração autônoma de livros de qualquer gênero, acompanhado de uma suíte de testes unitários locais rápidos.
...
AUTONOVEL_PROVIDER=openrouter
```

You can customize the model used for each phase (Writing, Judging, Reviewing) globally or per-provider. If provider-specific model variables are empty, they fall back to the generic `AUTONOVEL_WRITER_MODEL` (or respective judge/review var) or to the provider's curated default.
...
autonovel includes a comprehensive suite of **33 fast, deterministic local unit tests** located in the `tests/` directory.
