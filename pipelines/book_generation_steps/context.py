...
def count_total_chapters(outline_text: str) -> int:
    ...
    chapter_numbers = [int(chapter) for chapter in chapters_found]
    expected_numbers = list(range(1, max(chapter_numbers) + 1))
    if chapter_numbers != expected_numbers:
        raise ValueError(
            "Outline com numeracao de capitulos invalida. "
            f"Esperado sequencia {expected_numbers}, encontrado {chapter_numbers}."
        )
...
def load_lore_files(book_data_dir: Path) -> Dict[str, str]:
    ...
    required_files = {
        "world": book_data_dir / "world.md",
        "canon": book_data_dir / "canon.md",
        "characters": book_data_dir / "characters.md",
        "voice": book_data_dir / "voice.md",
    }
    missing = [path.name for path in required_files.values() if not path.exists()]
    if missing:
        missing_list = ", ".join(sorted(missing))
        raise FileNotFoundError(f"Arquivos obrigatorios de lore ausentes em {book_data_dir}: {missing_list}")
