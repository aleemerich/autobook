...
def call_review_llm(prompt, max_tokens=8000):
    ...
    system_prompt = (
        "You are a distinguished literary critic and a professor of creative writing. "
        "You evaluate manuscripts with professional objectivity, academic precision, and constructive feedback."
    )
    print(f"Sending manuscript to review LLM ({len(prompt):,} chars)...", file=sys.stderr)
    return call_llm(
        prompt=prompt,
