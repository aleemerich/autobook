...
> **Status:** o baseline moderno verificado e `uv run --with pytest pytest tests`, com 320 testes passando. A pasta `/legacy/tests` existe, mas foi desativada e excluída da suíte de testes moderna. Caso seja executada diretamente, a pasta é configurada para ser ignorada via `conftest.py`.
